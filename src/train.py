...
while True:
    ...
    tools.log(log_file_path, "Avg score over " + str(len(scores)) + " games of model trained on " + str(games_trained) + " games: " + str(round(sum(scores) / len(scores), 1)) + ", avg. score of best " + str(len(GamesToTrainOn)) + " games from that episode we will now train on: " + str(round(avg_score_best, 1)))
